...
function findError(where) {
  ...
  [...where.children].forEach((item) => result.push(item.textContent));
...
function deleteTextNodes(where) {
  [...where.childNodes]
    .reverse()
    .forEach((item) => item.nodeType === Element.TEXT_NODE && where.removeChild(item));
}
...
function deleteTextNodesRecursive(where) {
  [...where.childNodes].reverse().forEach((item) => {
    ...
    switch (item.nodeType) {
      case 1: {
        ...
        classes.forEach((classItem) => {
          if (!stat.classes[classItem]) {
            stat.classes[classItem] = 1;
          } else {
            stat.classes[classItem] += 1;
          }
        });
